...
function VoiceChat(props) {
  ...
  const setupWebRTCConnection = async (peerId) => {
    try {
      ...
      peerConnection.onicegatheringstatechange = () => {
        ...
        setConnectionStatus(`ICE gathering: ${peerConnection.iceGatheringState}`);
      };
  ...
  const startVoiceChat = async () => {
    try {
      ...
      if (connections && connections.length > 0) {
        ...
        for (const connection of connections) {
          try {
            ...
            console.log('Successfully set up connection with peer:', connection.peerId);
          } catch (err) {
            console.error('Failed to set up connection with peer:', connection.peerId, err);
          }
        }
      } else {
      ...
      setError(err.message || 'Could not start voice chat. Make sure you are connected to a peer first.');
    }
  };
    ...
    >
      ...
      {connectionStatus && (
        <div style={{
          marginBottom: "20px",
          textAlign: "center",
          color: connectionStatus.includes('connected') ? '#44ff44' : '#ffff44'
        }}>
          {connectionStatus}
        </div>
      )}
